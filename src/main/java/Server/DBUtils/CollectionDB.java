...
public class CollectionDB {
    ...
    public static String removeMASLBD(Integer meters_above_sea_level, String user) throws SQLException {
        Statement statement = connection.createStatement();
        StringBuilder sb = new StringBuilder();
        statement.execute("DELETE FROM " + collTable + " WHERE meters_above_sea_level ='" + meters_above_sea_level + "'AND login = '" + user + "'");
        ResultSet rs = statement.executeQuery("SELECT * FROM " + collTable + " WHERE meters_above_sea_level ='" + meters_above_sea_level + "'AND login != '" + user + "'");
        while (rs.next()) {
            sb.append(rs.getString(2) + ":id-" + rs.getLong(1)).append("\n");
        }
        statement.close();
